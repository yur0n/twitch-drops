...
twitch_miner = TwitchChannelPointsMiner(
    username=sys.argv[1],           # Twitch username
    password=sys.argv[2],         # If no password will be provided, the script will ask interactively
    init_token=sys.argv[3] if sys.argv[3] != 'False' else None,
    ...
    logger_settings=LoggerSettings(
        ...
        # ),
        webhook=Webhook(
            endpoint=os.getenv('WEBHOOK_ENDPOINT'),                                                                    # Webhook URL
            method="GET",                                                                   # GET or POST
            events=[Events.STREAMER_OFFLINE, 
					Events.DROP_CLAIM, Events.DROP_STATUS],                                  # Only these events will be sent to the endpoint
        ),
        # matrix=Matrix(
